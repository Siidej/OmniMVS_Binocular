...
class OmniMVS(nn.Module):
    def __init__(self, sweep, ndisp, min_depth, max_depth, w, h):
        ...
        self.inv_depths = np.linspace(1/max_depth, 1 / min_depth, ndisp) + np.finfo(np.float32).eps
        ...
        self.fusion = nn.Sequential(
            nn.Conv3d(32 * len(self.cam_list), 32, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True))
    ...
    def forward(self, batch):
        ...
        for cam_idx in cam_idxs:
            ...
            for d_idx, d in enumerate(self.depths[::2]):
                ...
                #warps[:, :, d_idx:d_idx + 1, :, :] = F.grid_sample(feat, grid, align_corners=True).unsqueeze(2)
